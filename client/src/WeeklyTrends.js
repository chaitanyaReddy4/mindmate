...
import { getLastNDates, getDateKey, getMessageEmotion, getShortDayLabel, getStressScore } from "./dashboardUtils";
...
function WeeklyTrends({ messages = [], darkMode = true }) {
  ...
  const chartData = useMemo(() => {
    ...
    const lastSevenDays = getLastNDates(7);

    const dailySummary = lastSevenDays.map((date) => {
      const key = getDateKey(date);
      const items = botMessages.filter((message) => getDateKey(message.time) === key);
      ...
      <div className="summary-card-header">
        <div>
          ...
          <p className="summary-card-copy">
            Message frequency and average stress are grouped by date for the last week.
          </p>
        </div>
      </div>
