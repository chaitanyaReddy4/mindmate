  ...
  const previousEntries = useMemo(
    () =>
      Object.entries(entries)
        .filter(([date, entry]) => date !== todayKey && Boolean(entry))
    ...
    <section className="summary-card journal-card">
      <div className="summary-card-header">
        <div>
          ...
          <h3 className="summary-card-title">Daily Journal</h3>
          <p className="summary-card-copy">
            Capture a quick reflection and keep a dated record of how each day
            felt.
          </p>
        </div>
      ...
      <div className="journal-layout">
        <div className="journal-editor">
          ...
          <div className="journal-actions">
            ...
            {savedMessage ? (
              <span className="journal-save-message">{savedMessage}</span>
            ) : null}
          </div>
        </div>
        ...
        <div className="journal-history">
          <div className="journal-preview-card">
            <h4 className="wellness-panel-title">Today's entry</h4>
                ...
                {previousEntries.map(([date, entry]) => (
                  <div key={date} className="journal-history-item">
                    ...
                    <p className="journal-history-copy">
                      {entry.length > 110 ? `${entry.slice(0, 110)}...` : entry}
                    </p>
                  </div>
                ))}
              </div>
